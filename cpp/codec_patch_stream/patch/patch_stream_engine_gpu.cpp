...
std::string resolve_decode_backend_for_gpu_module(const StreamConfig& cfg) {
  const std::string decode_backend =
      normalize_backend_key(cfg.decode_backend, "decode_backend");
...
std::string resolve_process_backend_for_gpu_module(const StreamConfig& cfg,
                                                   const std::string& resolved_decode_backend) {
  const std::string process_backend =
      normalize_backend_key(cfg.process_backend, "process_backend");
...
torch::Device output_device_for_cfg(const StreamConfig& cfg) {
  const std::string resolved_decode_backend = resolve_decode_backend_for_gpu_module(cfg);
  const std::string resolved_process_backend =
      resolve_process_backend_for_gpu_module(cfg, resolved_decode_backend);
  if (resolved_process_backend == "gpu") {
    return torch::Device(torch::kCUDA, cfg.process_device_id);
  }
...
at::Tensor normalize_per_frame_percentile_cpu(const at::Tensor& maps, double pct) {
  TORCH_CHECK(!maps.is_cuda(), "maps must be CPU tensor");
  TORCH_CHECK(maps.scalar_type() == at::kFloat, "maps must be float32");
  TORCH_CHECK(maps.dim() == 3, "maps must have shape (T,H,W)");
...
at::Tensor compute_energy_maps_cpu_local(const at::Tensor& frames_rgb_u8,
                                         const at::Tensor& mv_magnitude_maps,
                                         const std::vector<uint8_t>& is_i_positions,
                                         double energy_pct) {
  using torch::indexing::Slice;

  TORCH_CHECK(!frames_rgb_u8.is_cuda(), "frames_rgb_u8 must be CPU tensor");
  ...
  auto luma_small = at::upsample_bilinear2d(
                        luma.unsqueeze(1), {hs, ws}, false, c10::nullopt, c10::nullopt)
  ...
  auto residual_norm_small =
      normalize_per_frame_percentile_cpu(residual_proxy_small, energy_pct);
...
at::Tensor compute_energy_maps_cuda(const at::Tensor& frames_rgb_u8,
  ...
  TORCH_CHECK(!mv_magnitude_maps.defined() ||
                  (mv_magnitude_maps.is_cuda() &&
                   ...
                   mv_magnitude_maps.size(1) == h &&
                   mv_magnitude_maps.size(2) == w),
              "mv_magnitude_maps must be float32 CUDA tensor with shape (T,H,W)");
...
at::Tensor resize_to_input_cpu_local(const at::Tensor& frames_or_energy,
                                     int64_t target_h,
                                     int64_t target_w,
                                     bool is_energy) {
  TORCH_CHECK(!frames_or_energy.is_cuda(), "input must be CPU tensor");
  TORCH_CHECK(target_h > 0 && target_w > 0, "target_h/target_w must be > 0");
  ...
  auto resized =
      at::upsample_bilinear2d(x, {target_h, target_w}, false, c10::nullopt, c10::nullopt);
...
void CodecPatchStreamNative::prepare() {
  const std::string resolved_decode_backend = resolve_decode_backend_for_gpu_module(cfg_);
  const std::string resolved_process_backend =
      resolve_process_backend_for_gpu_module(cfg_, resolved_decode_backend);
  ...
  DecodeResultCanonical decoded = resolved_decode_backend == "gpu" ? decode_only_native_gpu(req)
                                                                   : decode_only_native_cpu(req);
  ...
  at::Tensor frames_for_process = decoded.frames_rgb_u8.contiguous();
  at::Tensor mv_maps_for_process = decoded.mv_magnitude_maps.defined()
                                       ? decoded.mv_magnitude_maps.contiguous()
                                       : at::Tensor();

  at::Tensor visible;
  at::Tensor scores_for_visible;
  int64_t hb = 0;
  int64_t wb = 0;

  if (resolved_process_backend == "gpu") {
    const auto process_device = torch::Device(torch::kCUDA, cfg_.process_device_id);
    if (!frames_for_process.is_cuda() || frames_for_process.device() != process_device) {
      frames_for_process =
          frames_for_process.to(process_device, /*non_blocking=*/false).contiguous();
    }
    if (mv_maps_for_process.defined() &&
        (!mv_maps_for_process.is_cuda() || mv_maps_for_process.device() != process_device)) {
      mv_maps_for_process =
          mv_maps_for_process.to(process_device, /*non_blocking=*/false).contiguous();
    }

    auto energy = compute_energy_maps_cuda(
        frames_for_process, mv_maps_for_process, decoded.is_i_positions, cfg_.energy_pct);
    energy = resize_to_input_cuda(energy, target_h, target_w, true);

    auto resized_frames =
        resize_to_input_cuda(frames_for_process.to(at::kFloat), target_h, target_w, false);
    ...
    visible = selection.visible_indices.contiguous();
    scores_for_visible = selection.scores_flat;
    hb = selection.hb;
    wb = selection.wb;
  } else {
    if (frames_for_process.is_cuda()) {
      frames_for_process =
          frames_for_process.to(torch::kCPU, /*non_blocking=*/false).contiguous();
    }
    if (mv_maps_for_process.defined() && mv_maps_for_process.is_cuda()) {
      mv_maps_for_process =
          mv_maps_for_process.to(torch::kCPU, /*non_blocking=*/false).contiguous();
    }

    auto energy = compute_energy_maps_cpu_local(
        frames_for_process, mv_maps_for_process, decoded.is_i_positions, cfg_.energy_pct);
    energy = resize_to_input_cpu_local(energy, target_h, target_w, true);

    auto resized_frames =
        resize_to_input_cpu_local(frames_for_process.to(at::kFloat), target_h, target_w, false);

    auto selection = compute_visible_indices_cpu(energy,
                                                 cfg_.patch_size,
                                                 cfg_.k_keep,
                                                 cfg_.selection_unit,
                                                 cfg_.static_fallback,
    ...
    auto patches_f32 = extract_patches_by_indices_cpu(resized_frames.contiguous(),
                                                      selection.visible_indices.contiguous(),
                                                      cfg_.patch_size,
                                                      selection.wb);
    patch_bank_ = patches_f32.to(cfg_.output_dtype);
  ...
  const int64_t ppf = hb * wb;

  const torch::Device metadata_device =
      resolved_process_backend == "gpu"
          ? torch::Device(torch::kCUDA, cfg_.process_device_id)
          : torch::Device(torch::kCPU);
  auto long_opts = torch::TensorOptions().dtype(at::kLong).device(metadata_device);
  ...
  auto patch_h_lut = torch::arange(hb, long_opts).repeat_interleave(wb).contiguous();
  auto patch_w_lut = torch::arange(wb, long_opts).repeat({hb}).contiguous();
  ...
  auto frame_ids_cpu = torch::from_blob(const_cast<int64_t*>(decoded.sampled_frame_ids.data()),
                           ...
                           .clone();
  auto frame_ids = frame_ids_cpu.to(metadata_device, /*non_blocking=*/false).index_select(0, seq_pos);

  auto is_i_cpu = torch::from_blob(const_cast<uint8_t*>(decoded.is_i_positions.data()),
                                   {t},
                                   torch::TensorOptions().dtype(at::kByte))
                      .clone();
  auto is_i = is_i_cpu.to(metadata_device, /*non_blocking=*/false)
                  .toType(at::kLong)
                  .index_select(0, seq_pos);

  metadata_fields_gpu_ =
      torch::stack({seq_pos, frame_ids, is_i, visible, ph_idx, pw_idx}, 1).contiguous();
  metadata_scores_gpu_ =
      scores_for_visible.index_select(0, visible).toType(at::kFloat).contiguous();
...
std::tuple<at::Tensor, std::vector<PatchMeta>> CodecPatchStreamNative::next_n(int64_t n) {
  if (n <= 0 || !patch_bank_.defined() || closed_ || cursor_ >= size()) {
    const auto output_device = output_device_for_cfg(cfg_);
