...
at::Tensor select_units_with_iframe_priority_cuda(const at::Tensor& unit_scores,
  ...
  auto long_opts = torch::TensorOptions().dtype(at::kLong).device(unit_scores.device());
  auto float_opts = torch::TensorOptions().dtype(at::kFloat).device(unit_scores.device());
...
SelectionResult compute_visible_indices_cuda(const at::Tensor& energy,
  ...
  if (selection_unit == 0) {
    ...
    auto visible = select_units_with_iframe_priority_cuda(scores_flat,
                                                          patch_ids,
                                                          patch_frames,
                                                          is_i_frame,
                                                          k_global,
  ...
  auto selected_blocks = select_units_with_iframe_priority_cuda(block_scores,
                                                                block_ids,
                                                                block_frames,
                                                                is_i_frame,
                                                                k_blocks,
